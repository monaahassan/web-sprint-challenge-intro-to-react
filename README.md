...
React JS is a library created by faceboook. The main concept for React JS is to use a virtual DOM to communicate with the real DOM in order to render more quickly than if we were to use just our DOM.

1. Describe component state.
A component is a resuable UI on the page. So lets say we have twitter for ex, the dashboard is on every page, so instead of rendering a whole page, we create a component of just the dashboard. 

1. Describe props.
Props are just properties that are accepted as input. 


1. What are side effects, and how do you sync effects in a React component to changes of certain state or props?
Side effects is anything outside of the function that renders the function to change into something else. We use useEffect to sync pages. 
